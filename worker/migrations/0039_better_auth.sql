...
ALTER TABLE workers ADD COLUMN better_auth_user_id TEXT REFERENCES "user" ("id") ON DELETE SET NULL;
ALTER TABLE workers ADD COLUMN must_change_password INTEGER NOT NULL DEFAULT 0 CHECK (must_change_password IN (0, 1));
CREATE UNIQUE INDEX workers_better_auth_user_id_idx ON workers (better_auth_user_id) WHERE better_auth_user_id IS NOT NULL;
